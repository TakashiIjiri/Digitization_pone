...
public :
	...
	inline void setPix( int id4, EVec4i c){
		m_RGBA[id4+0] = c[0];
		m_RGBA[id4+1] = c[1];
		m_RGBA[id4+2] = c[2];
		m_RGBA[id4+3] = c[3];
	...
	inline EVec4i getRGBA(int i4) const
	{
		return EVec4i( m_RGBA[i4+0], m_RGBA[i4+1], m_RGBA[i4+2], m_RGBA[i4+3]);
	}
	...
	inline bool isBlackPix(const int &i4) const 
	{
		return  m_RGBA[i4+0] == 0 && 
				m_RGBA[i4+1] == 0 && 
				m_RGBA[i4+2] == 0 ;
	}
