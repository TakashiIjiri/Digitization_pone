...
// TexMapper.cpp : アプリケーションのクラス動作を定義します。
//
...
BEGIN_MESSAGE_MAP(CTexMapperApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CTexMapperApp::OnAppAbout)
...
// CTexMapperApp コンストラクション

CTexMapperApp::CTexMapperApp()
{
	// 再起動マネージャーをサポートします
	...
	SetAppID(_T("TexMapper.AppID.NoVersion"));
...
// 唯一の CTexMapperApp オブジェクトです。

CTexMapperApp theApp;


// CTexMapperApp 初期化

BOOL CTexMapperApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CTexMapperDoc),
		RUNTIME_CLASS(CMainFrame),       // メイン SDI フレーム ウィンドウ
		RUNTIME_CLASS(CTexMapperView));
	if (!pDocTemplate)
		return FALSE;
...
int CTexMapperApp::ExitInstance()
{
